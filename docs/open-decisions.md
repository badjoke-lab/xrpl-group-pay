...
| OD-010 | Primary XRPL endpoints and failover? | One configured primary and one independent failover per network | Reliability and rate-limit review | Validated-ledger verification PR | OPEN |
...
| OD-013 | Destination Tag behavior? | Optional input; exact match when present | Recipient account checks and verification tests | Validated-ledger verification PR | ASSUMPTION |
