...
export function classifyMainnetD1State(config, policy) {
  ...
  throw new Error(
    "D1 provisioning requires the committed safe default ALLOW_MAINNET_RUNTIME=false or the reviewed internal halted state.",
  );
}
...
export function resolveDatabasePair(
  databases,
  policy,
  { requireBoth = true } = {},
) {
...
export function assertProvisioningRequest(
  policy,
  { mode, confirmation, location },
) {
...
export async function runMainnetD1Provisioning(options = {}) {
  ...
  const [policy, wranglerSource] = await Promise.all([
    readFile(policyPath, "utf8")
      .then(JSON.parse)
      .then((value) => policySchema.parse(value)),
  ...
  if (pair.production && pair.preview) {
    const provisionedConfig = createProvisionedWrangler(
      wranglerSource,
      policy,
      {
    ...
    if (mode === "provision") {
      ...
      if (previewInspection.migration_count !== migrationFiles.length) {
        throw new Error(
          "Preview D1 migration count does not match the repository.",
        );
      }
      ...
      if (productionInspection.migration_count !== migrationFiles.length) {
        throw new Error(
          "Production D1 migration count does not match the repository.",
        );
      }
