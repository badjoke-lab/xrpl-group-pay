...
const REDEPLOYMENT_STAGES = new Set([
  "live-xrp-acceptance",
...
function isClosedInitialConfiguration(vars) {
  return (
...
function isReviewedHaltedConfiguration(mainnet, vars, contract) {
  return (
    ...
    mainnet.routes?.some(
      (route) =>
        route.pattern === contract.custom_domain && route.custom_domain === true,
    ) &&
