...
test("renders the Xaman Testnet payment vertical slice", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Send one XRP Payment through Xaman/i })).toBeVisible();
  await expect(page.getByLabelText("Recipient XRPL address")).toBeVisible();
  await expect(page.getByRole("button", { name: "Continue to Xaman" })).toBeVisible();
...
test("exposes a no-store health endpoint", async ({ request }) => {
  ...
  await expect(response.json()).resolves.toMatchObject({ status: "ok", service: "xrpl-group-pay", network: "testnet" });
});
