...
describe("GuideBrowser", () => {
  it("filters complete Guide sections without changing stable anchors", () => {
    ...
    expect(screen.getByRole("heading", { name: "Official RLUSD TrustSet preparation" })).toBeVisible();
    expect(screen.getByRole("link", { name: "Official RLUSD TrustSet preparation" })).toHaveAttribute(
      "href",
      "#trustset",
    );
    expect(screen.queryByRole("heading", { name: "Bill and payer progress" })).toBeNull();
  });
  ...
  it("shows a recoverable no-results state", () => {
    ...
    expect(screen.getByRole("heading", { name: "一致するガイド項目がありません" })).toBeVisible();

    fireEvent.click(screen.getByRole("button", { name: "検索をクリア" }));
    expect(screen.getByRole("heading", { name: "目的と非カストディ型の仕組み" })).toBeVisible();
  });
